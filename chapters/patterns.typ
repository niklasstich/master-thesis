...
In cases where the adapting is purely name-related or the transformations that need to be done to adapt the interface onto the existing implementations are trivial enough to represent with a simple lambda (for example, adapting radians to degrees or polar coordinates to cartesian coordinates), it could be feasible to implement a set of aspects that automatically generates these adapter classes for us. 

The problem the adapter addresses is also yet another cross-cutting concern, as we are not concerned with the business logic of the code that we are adapting, but instead merely trying to bring together two different sets of requirements in terms of implementation details, namely the concrete publicly-facing interface contracts of the client and the dependency.
