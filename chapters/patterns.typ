...
We definitely have to ask the question, however, what we would achieve by doing all this. The command pattern in its form described here already fits perfectly within the tools of abstraction that object-oriented programming offers us: there are no cross-cutting concerns to speak of, rather the concern that the command pattern addresses is already abstracted into its own class, the concrete command itself. Sure, we would still save ourselves the copy-and-paste work of creating these command classes ourselves, but it comes at the massive cost of reduced customizability as described above. If we, in the future, want to move from the thin command objects to the more involved philosophy of putting all the logic from the receiver into the command, we cannot do that with the automatic command implementation #footnote([A possible solution for this can be the Metalama feature of divorcing where we can remove our reliance on a certain aspect by generating out the source code it produces once and adding it into our own codebase, see @metadocs[Divorcing from Metalama]]).
