...
The singleton pattern is a creational pattern which can be used to "[e]nsure a class only has one instance, and provide a global point of access to it"@Gamma1994[p. 127]. At any point of a program's lifecycle, if a class implements the singleton pattern, there must, at most, be one instance of said class. To ensure that this is the case, we make the singleton class itself responsible for managing its own instance creation and object lifecycle as well as providing access to this instance@Gamma1994[p. 127] by declaring all constructors of the type as non-public and providing a public-facing static operation to retrieve the instance of the type@Gamma1994[p. 129]. This static operation must check whether there is already an instance of the type stored statically inside of the type, create a new instance if not, and return the single instance@Gamma1994[p. 129 implementation].
...
To solve this, we can use lazy initialization as shown in @singleton_lazy_initialization. Instead of creating the instance of our singleton type directly in our static constructor and storing it in the `Instance` property, we use the `Lazy<T>` type from the .NET Core API to lazily instantiate our type@dotnetdocs[Lazy\<T> Class]. In this setup, we do not have to care about when the static constructor is called, as calling the static instructor does not instantiate the type directly; it is instead only created (via the provided lambda inside the `Lazy` constructor call) when we try to retrieve the value from the `Lazy` object via the `Instance` computed property. One advantage of using `Lazy<T>` for the initialization logic is that it is already thread-safe out of the box@dotnetdocs[Lazy\<T> class]. To be specific, the `Lazy<T>` constructor shown in the example sets the thread safety mode to `LazyThreadSafetyMode.ExecutionAndPublication`, which means that in case `.Value` is called from multiple threads simultaneously, only one thread will ever be allowed to call the provided initialization lambda@dotnetdocs[LazyThreadSafetyMode Enum].
...
First of all, the aspect retrieves all constructors on the target type. If there is exactly one constructor, we check if that constructor is the implicit parameterless constructor of the type. In C\#, any non-static class that has no explicitly defined constructors will automatically get an implicit public constructor with no parameters@dotnetdocs[Using Constructors]. If that is the case, we will warn the user that this type has no explicit constructor, and therefore has the implicit public parameterless constructor, meaning it is technically not suitable for the singleton pattern as there should be no publicly accessible constructors on the type. Otherwise, we go through all constructors again and check whether they are public and accordingly report those as warnings too, as they should not be public to prevent the singleton from being instantiated anywhere else. After this, we go to the actual generating step of the singleton, which depends on the value set to the `Lazy` parameter mentioned before. The code up to this step can be seen in @singleton_aspect_analysis.
...
#codly(
  highlights: (
    ...
    ct(26, start: 44, tag: "[2]", label: <singleton_lazy_new_constraint>),
...
The non-lazy implementation is essentially the same as the lazy implementation from @singleton_lazy_implementation, with the key differences being that we use the target type directly instead of the lazy generic type when we define the `_instance` field and its initializer, and dropping the `.Value` access as we've dropped the indirection. Otherwise, the steps are exactly the same and the final generated code looks the same to our consumers interface-wise.
...
Using our singleton aspect to implement the singleton functionality for us does not give us quite as many advantages as the memento aspect did, because singleton is a pretty static pattern that does not change much even when the type we implement singleton itself changes significantly. That does however not mean that using metaprogramming to extract the singleton implementation from our singleton types itself has no merit. The arguments put forward in @memento_consequences, except for the argument about the changing implementation, still stand. By removing the singleton implementation from the explicit code of the type we have made the type itself easier to understand and maintain, fulfilling our adapted definition of the SRP. We've also again reduced code duplication by not having to copy-paste the same singleton implementation every time, instead relying on one singleton aspect implementation which can be tested in advance, reducing run-time code test requirements and which can be changed once centrally to change the implementation of all our singletons automatically.

It can therefore be concluded that even though the singleton initially appears very simple to understand and implement, there are still advantages to be had from automating it, even though it is "only" a creational pattern and does not carry much logic itself. This is because the singleton pattern solves a problem that fits our definition of cross-cutting concerns from @aop very well: the logic of the singleton pattern is concerned with the lifetime of objects of our target types rather than the actual functional business logic of the types itself, compare with @Kiczales1997[definition on p. 7]. If we follow the theory of aspect-oriented programming@Kiczales1997, it is only natural that we'd want to pull this concern out of our components.
