...
In earlier versions of Metalama before the release of 2024.2, it was not possible to introduce new classes to a compilation. This meant that the user had to provide the nested memento type predefined in their code for us, which would then be filled with the implementation at compile-time. The `BuildEligibility` method of `MementoAttribute` would check for the presence of this type and would report a compile error diagnostic if it were missing. Since said Metalama version was released however, we can now simply introduce the type ourselves as shown in @memento_configure_child, so this check could be removed. The user's ability to define the type themselves was retained however, as to give the user the option to add custom fields or properties to the memento type for use in create and restore hook methods.
