...
The pattern described in this section is not a new invention, and we lay no claim to inventing or formalizing it, however, it is very difficult to find any substantial literature on it#footnote([The closest thing that can be found is the "dirty flag" pattern, which is often used in games or other applications where performance is critical, to only perform certain operations exactly when they are needed. Both the problem that's being solved with that pattern and the solution to the problem are different to what will be explained here because we are more concerned with notifying our UI whether *any* object in our hierarchy has changed rather than saving operations at an object level. An analysis of that pattern can be found at https://gameprogrammingpatterns.com/dirty-flag.html.]). As such, we will first describe the problem we're trying to solve here, then propose a systematic solution to it, before going into how to automate implementing it.

== Analysis of Pattern<unsaved_changes_analysis>


In modern applications that allow users to edit files of any sort, it is very common to give users an indication whenever a file or open context of any sort has any changes that have not been saved yet. Two different examples can be observed in @unsaved_changes_example_images. There are many different ways we can handle a user's unsaved changes, the first example is a blocking modal dialogue that prevents the user from closing the application until they select how they want to proceed, and the second example is a much more subtle way of informing the user, which is acceptable in this scenario, as the application in question actually persists unsaved changes when it is closed and reopened.
...
```, caption: [Example of a basic weapon class with UnsavedChanges property]
...
The usage of many different other patterns could be discussed here, for example, if we only cared about the fact that *an* object in our hierarchy has changed but never cared about *what part of the object hierarchy* has changed, we could use a mediator@Gamma1994[p. 273ff] to extract and simplify the logic of change notification by making the Player class (or whatever the top object of our hierarchy ends up being) subscribe to the `OnChange` event of some sort of `ChangeTracker` mediator class, passing a reference to this mediator to every member of the hierarchy and making the components call this mediator whenever there is a change, effectively notifying our Player. Using this approach we could find out which concrete object has changed (simply by making the changed object pass itself to the mediator, which passes it to the player), but it would be difficult to identify which subtree of our object structure has changed because of this, which could be something we need if we need to persist subtrees of our hierarchy on change for example. Using C\# delegates@dotnetdocs[Delegates (C\# Programming Guide)] could even eliminate the need for the mediator type. One could also think about whether the application of the chain of responsibility pattern brings advantages here, but we will do without discussing whether this improves the proposed design in this thesis and move forward with how to automatically implement said design.
...
Finally, in @unsaved_changes_implementation_part4, we introduce the `ResetUnsavedChanges` method via another template.This template is not shown here for brevity, but it essentially does the same thing as the `GetUnsavedChanges` template by going over all members and enumerable members and calling `ResetUnsavedChanges` on the objects. The `UnsavedChanges` property is introduced via a final template that simply calls the `GetUnsavedChanges()` method on the instance. We want these to be public, as these are the public-facing contracts of the `IUnsavedChanges` interface.
#pagebreak(weak: true)
#codly(
  highlights:
  (
...
caption: [Data structure for @unsaved_changes_diff. The backslash in member `Bs2` in class `A` is needed because of a PlantUML limitation.]
