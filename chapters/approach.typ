...
In contrast to AspectJ, these aspects don't define themselves where they are applied via the joinpoints introduced earlier. Instead, we mark the declarations we want to manipulate directly with aspects (which are C\# attributes as seen in @metalama_classdiag)

The `BuildEligibility` method allows us to check whether or not the aspect that is being executed is actually applicable to the target declaration. To define these checks, we can use predefined extension methods like the `MustNotBeInterface` or `MustNotBeAbstract` or define our own logic using the `MustSatisfy` method that takes in a validation predicate and a function that generates a justification for why a target is not eligible for the aspect#footnote([See https://doc.postsharp.net/metalama/api/metalama-framework-eligibility-eligibilityextensions for the mentioned extension methods]). If the eligibility is true, the `BuildAspect` method is executed in which we can do our actual aspect logic such as introducing new members, types or methods, implementing interfaces, changing existing code and much more. 
